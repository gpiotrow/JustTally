...
describe('sessionsToCsv', () => {
  it('emits a header followed by one row per set', () => {
    ...
    expect(lines[0]).toBe('date;session_title;exercise_name;set_number;type;reps;weight_kg;rpe;done');
  });

  it('fills type, rpe and done-ness per set', () => {
    const csv = sessionsToCsv([session]);
    const lines = csv.trim().split('\n');
    expect(lines[1]).toBe('"2026-01-15";"Push A";"Bench Press";"1";"warmup";"10";"40";"";"true"');
    expect(lines[2]).toBe('"2026-01-15";"Push A";"Bench Press";"2";"working";"8";"80";"8";"true"');
    expect(lines[3]).toBe('"2026-01-15";"Push A";"Bench Press";"3";"drop";"6";"62.5";"";"false"');
  });
