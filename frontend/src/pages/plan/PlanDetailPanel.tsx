...
import { TRACKING_FIELDS, type TrackingMode } from '../../lib/tracking';
 ...
 *
 * Which target fields appear depends on the exercise's own tracking mode
 * (`exercises`, the catalog): a plank offers a duration target instead of
 * reps and weight, a run offers distance and duration instead of reps.
 */
export function PlanDetailPanel({
  ...
  const mode: TrackingMode = catalogExercise ? exerciseTracking(catalogExercise) : 'reps_weight';
  ...
  return (
    <div className="h-full space-y-4 overflow-y-auto border-l border-border p-4">
      ...
      {fields.includes('duration') && (
        <div>
          <label className="label" htmlFor="detail-duration">
      ...
      {fields.includes('distance') && (
        <div>
          <label className="label" htmlFor="detail-distance">
          ...
          <input
            ...
            min={0}
            className="input"
            value={exercise.targetDistanceM ?? ''}
            onChange={(e) =>
              onChange({
                targetDistanceM: e.target.value === '' ? undefined : Math.max(0, Number(e.target.value)),
              })
            }
          />
        </div>
